...
def plot_landmark(landmarks, side_length=256, draw_head=True, save=False):
    import matplotlib
    if not cuda:
    ...
    # plt.savefig("landmark_test/test_1.png")
    plt.close(fig)
    return data

def plot_landmark_save_frames(landmarks, side_length=256, draw_head=True, frame_number=0):
    import matplotlib
    if not cuda:
        matplotlib.use('TkAgg')
    else:
    ...
    data = PIL.Image.frombuffer('RGB', fig.canvas.get_width_height(), fig.canvas.tostring_rgb(), 'raw', 'RGB', 0, 1)
    plt.savefig("landmark_test/pose_{}.png".format(frame_number))
    plt.close(fig)
    return data
...
def get_landmark_save_frames(image, from_tensor=False, draw_head=True, frame_number=0):
    transform = Compose([ToTensor(), Normalize([0.5,0.5,0.5],[0.5,0.5,0.5])])
    if from_tensor:
        image = (image + 1) / 2
        image = np.array(ToPILImage()(image))
    ...
    return transform(plot_landmark_save_frames(pred, draw_head=draw_head, frame_number=frame_number))
...
def get_frames_and_landmarks_non_random(video_path, num_frames, generate_landmark=False):
    augumentation = Compose([RandomHorizontalFlip(0.5)])
    transform = Compose([ToTensor(), Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])])
    res = []
    cap = cv2.VideoCapture(video_path)
    ...
    for frame_number in range(0, num_frames):
        ...
        if generate_landmark:
            landmark = get_landmark_save_frames(frame, frame_number=frame_number)
            res.append([transform(frame), landmark])
        else:
            res.append(transform(frame))
    cap.release()
