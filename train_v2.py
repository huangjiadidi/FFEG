...
model = Model(multiscaled=False).to(device)
...
for e in range(150):
    ...
    for i, batch in enumerate(training_data_loader):
        ...
        if i % save_interval == save_interval - 1:
            res = model.train_one_dis(source, target, source_pose, target_pose, other_pose, same_person_pose, target_mask, index, opt.k, i, get_image=True)
        else:
            res = model.train_one_dis(source, target, source_pose, target_pose, other_pose, same_person_pose, target_mask, index, opt.k, i)
